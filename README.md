...
- **Fee or interest** on any transaction — an optional field on Add
  Transaction (and on Investment contributions) that posts as its own
  linked entry in the same account: negative for a fee/charge, positive
  for interest, filed under new **Fees & Charges** / **Interest**
  categories so your real transaction amount stays untouched.
- **Navigation hub:** there's no side menu — every section is reachable from
  the icon grid on the **Dashboard**, between the balance card and Recent
  Transactions. Any other page shows a "🏠 Dashboard" button top-right to
  get back.
- **Debts & Loans history:** once a debt or loan is fully paid off, it
  automatically moves from the active list into a **History** section
  showing it as "✓ Completed", with its full payment/collection history.
- **Light and dark themes** — switch anytime in **Settings → Theme**. Applies
  instantly across every page and is remembered on your device.
- Rounded card-based design (dashboard stat cards, donut/bar charts, pill
