...
const DEFAULT_CATEGORIES = [
  ...
  { id: "interest", name: "Interest", type: "both" },
...
function render() {
  ...
  if (homeBtn) homeBtn.style.display = hash === "dashboard" ? "none" : "flex";
  ...
  return `
    ...
    <div class="icon-grid">
      ${NAV.filter((n) => n.id !== "dashboard").map((n) => `
        <button class="icon-tile" data-action="goto" data-hash="${n.id}">
          <span class="glyph-circle">${n.glyph}</span>
          <span class="tile-label">${esc(n.label)}</span>
        </button>`).join("")}
    </div>
...
function defaultAddForm() {
  return { type: "expense", amount: "", date: nowLocalInput(), category: "food", accountId: state.accounts[0]?.id || "", toAccountId: "", description: "", recurring: false, frequency: "monthly", debtPartyName: "", linkDebtId: "", feeInterest: "" };
}
...
function pageAdd() {
  ...
  if (!ui.addForm) {
    ui.addForm = editing
      ? { type: editing.type, amount: String(editing.amount), date: editing.date, category: editing.category, accountId: editing.accountId, toAccountId: editing.toAccountId || "", description: editing.description || "", recurring: !!editing.recurring, frequency: editing.frequency || "monthly", debtPartyName: "", linkDebtId: editing.debtId || "", feeInterest: "" }
      : defaultAddForm();
  ...
  return `
    ...
    <form class="ledger-form" data-form="add-transaction">
      ...
      <label>Fee or interest on this transaction (optional)
        <input type="number" step="0.01" name="feeInterest" value="${esc(f.feeInterest)}" placeholder="e.g. -22 for a fee, 150 for interest earned" />
      </label>
      <p class="muted" style="font-size:11px;margin:-8px 0 0">Posted as its own linked entry in the same account — negative for a fee/charge, positive for interest.</p>
...
function syncAddFormFromDOM() {
  ...
  ui.addForm.feeInterest = fd.get("feeInterest") ?? ui.addForm.feeInterest;
}
...
function debtListHTML(list, { showDirection = false } = {}) {
  ...
  return list.map((d) => {
    ...
    return `<div class="ledger-row">
      <div style="flex:1">
        <div class="row-title">${esc(d.name)}
          ${showDirection ? `<span class="tag txt-mono">${d.direction === "owe" ? "debt" : "loan"}</span>` : ""}
          ${d.accountId ? `<span class="tag txt-mono">${esc(accName(d.accountId))}</span>` : ""}
          ${d.dueDate ? `<span class="tag txt-mono">due ${esc(fmtDate(d.dueDate))}</span>` : ""}
          ${isDone ? `<span class="liquid-tag yes">✓ Completed</span>` : ""}
        </div>
        <div class="progress-track"><div class="progress-fill" style="width:${(d.paid / d.amount) * 100 || 0}%"></div></div>
        <div class="row-sub txt-mono muted">${fmtShort(d.paid)} paid of ${fmtShort(d.amount)} · ${isDone ? "settled in full" : fmtShort(remaining) + " remaining"}</div>
...
function pageDebts() {
  ...
  const oweActive = owe.filter((d) => d.amount - d.paid > 0);
  const owedActive = owed.filter((d) => d.amount - d.paid > 0);
  const history = state.debts.filter((d) => d.amount - d.paid <= 0).sort((a, b) => (b.paid) - (a.paid));
  const totalOwe = oweActive.reduce((s, d) => s + (d.amount - d.paid), 0);
  const totalOwed = owedActive.reduce((s, d) => s + (d.amount - d.paid), 0);
  ...
  return `
    ...
    <div class="section-title">You owe</div><div class="ledger-list">${debtListHTML(oweActive)}</div>
    <div class="section-title">Owed to you</div><div class="ledger-list">${debtListHTML(owedActive)}</div>
    <div class="section-title">History — completed debts &amp; loans</div>
    <div class="ledger-list">${history.length ? debtListHTML(history, { showDirection: true }) : emptyHTML("Nothing paid off yet — settled debts and collected loans will show up here.")}</div>`;
}
      ...
      ${invAccounts.length === 0 ? emptyHTML("Mark an account as 'investment' type in Accounts to see it here.") : invAccounts.map((a) => `
        <div class="account-card">
          ...
          ${ui.contributingInvId === a.id
            ? `<form class="inline-form" style="margin-top:8px;flex-direction:column;align-items:stretch" data-form="contribute-invest" data-id="${a.id}">
                <input type="number" name="amount" placeholder="Contribution amount" required />
                <input type="number" step="0.01" name="feeInterest" placeholder="Fee or interest (optional, e.g. -50 or 120)" />
                <button class="btn btn-primary" type="submit">Add</button>
              </form>`
...
function pageSettings() {
  ...
  return `
    ...
    <p class="muted">Accounts have their own page — reachable from the Dashboard icon grid.</p>
...
main.addEventListener("submit", (e) => {
  ...
  if (form.dataset.form === "add-transaction") {
    ...
    update((s) => {
      ...
      if (!editing && feeInterestAmt !== 0) {
        const feeRecord = {
          id: uid(), type: feeInterestAmt < 0 ? "expense" : "income", amount: Math.abs(feeInterestAmt),
          date: f.date, category: feeInterestAmt < 0 ? "fees" : "interest", accountId: f.accountId, toAccountId: null,
          description: (feeInterestAmt < 0 ? "Fee on: " : "Interest on: ") + (f.description || catName(f.category)),
          recurring: false, relatedTxId: record.id,
        };
        applyTxEffect(feeRecord, 1);
        s.transactions.push(feeRecord);
      }
    });
  ...
  if (form.dataset.form === "contribute-goal") {
    ...
    if (amt) update((s) => { const g = s.goals.find((x) => x.id === goalId); g.current += amt; g.contributions = g.contributions || []; g.contributions.push({ date: todayISO(), amount: amt }); });
    else render();
  }
  ...
  if (form.dataset.form === "contribute-invest") {
    const amt = parseFloat(fd.get("amount"));
    const feeInterestAmt = parseFloat(fd.get("feeInterest")) || 0;
    const accId = form.dataset.id;
    const now = nowLocalInput();
    ui.contributingInvId = null;
    if (amt) update((s) => {
      const a = s.accounts.find((x) => x.id === accId);
      a.balance += amt;
      s.transactions.push({ id: uid(), type: "income", amount: amt, date: now, category: "investment", accountId: a.id, toAccountId: null, description: "Investment contribution", recurring: false });
      if (feeInterestAmt !== 0) {
        a.balance += feeInterestAmt;
        s.transactions.push({
          id: uid(), type: feeInterestAmt < 0 ? "expense" : "income", amount: Math.abs(feeInterestAmt),
          date: now, category: feeInterestAmt < 0 ? "fees" : "interest", accountId: a.id, toAccountId: null,
          description: (feeInterestAmt < 0 ? "Fee on: " : "Interest on: ") + "Investment contribution", recurring: false,
        });
      }
    });
    else render();
